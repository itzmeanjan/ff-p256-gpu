...
void
test_matrix_transposed_initialise(sycl::queue& q,
                                  const uint64_t dim,
                                  const uint64_t wg_size)
{
  ...
  ff_p254_t* vec_dst =
    static_cast<ff_p254_t*>(sycl::malloc_shared(sizeof(ff_p254_t) * n * n, q));

  sycl::event evt_0 = q.memset(vec_dst, 0, sizeof(ff_p254_t) * n * n);
  prepare_random_vector(vec_src, dim);
  matrix_transposed_initialise(
    q, vec_src, vec_dst, n2, n1, n, wg_size, { evt_0 })
    .wait();
...
void
test_matrix_transpose(sycl::queue& q,
                      const uint64_t dim,
                      const uint64_t wg_size)
{
...
void
test_twiddle_multiplication(sycl::queue& q,
                            const uint64_t dim,
                            const uint64_t wg_size)
{
  assert((dim & (dim - 1ul)) == 0);
  ...
  sycl::event evt_2 = twiddle_multiplication(
    q, vec_src, omega, n2, n1, n, wg_size, { evt_0, evt_1 });
...
void
test_six_step_fft_ifft(sycl::queue& q,
                       const uint64_t dim,
                       const uint64_t wg_size)
{
  assert((dim & (dim - 1ul)) == 0);
